...
var FLOOR_LIMITER_NAMESPACE = settingsNamespace("floor-limiter");
var FloorLimiterSettingsSchema = Schema.object({
  enabled: Schema.boolean().default(true),
  /** Trigger: compact when this many real user floors are on the surface. */
  triggerFloors: Schema.number().step(1).min(1).default(20),
  /** Keep verbatim: the newest N floors are left untouched; only older ones are compacted. */
  keepFloors: Schema.number().step(1).min(0).default(5)
});
function registerFloorLimiterSettings(ctx) {
...
function isUserFloor(event) {
  return event?.type === "user/message" && event.data.source.kind === USER_SOURCE_KIND;
}
function seamSession(session) {
  return session;
}
function countUserFloors(session) {
...
function selectCompactionRange(session, keepFloors) {
  const nodes = session.surface.nodes;
  if (nodes.length === 0) return null;
  const events = session.events;
  if (events === void 0) return null;
  const bySeq = /* @__PURE__ */ new Map();
  for (const event of events) bySeq.set(event.seq, event);
  const floorBegins = [];
  let inFloor = false;
  for (let index = 0; index < nodes.length; index += 1) {
    ...
    if (event !== void 0 && isUserFloor(event)) {
  ...
  let endIdx = keepFloors === 0 ? nodes.length - 1 : floorBegins[keepFloorOffset] - 1;
  while (endIdx >= 0) {
    let balanced;
    try {
...
async function maybeCompactSession(ctx, agent, settings, signal, compaction) {
...
// src/index.ts
var name = "floor-limiter";
var inject = ["settings", "agents", "agentPresets"];
function apply(ctx) {
